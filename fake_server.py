...
def _load_config() -> dict[str, Any]:
    ...
    back to an empty config (firmware defaults).
    """
    ...
    try:
        with open(built_config, "r", encoding="utf-8") as f:
            data = json.load(f)
            print(f"Config loaded from {os.path.relpath(built_config, SCRIPT_DIR)}")
            return data
    except FileNotFoundError:
        print(f"Built config not found at {os.path.relpath(built_config, SCRIPT_DIR)}, using empty config")
        return {}
